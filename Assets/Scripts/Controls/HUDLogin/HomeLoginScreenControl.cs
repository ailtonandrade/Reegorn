...
public class HomeLoginScreenControl : AuthenticateService
{
    ...
    void onClickButtonPlay()
    {
        GameObject.Find("ButtonPlay").GetComponent<Button>().onClick.AddListener(async () =>
        {
            ...
            {
                validateAccess(user, await response.Content.ReadAsStringAsync());
            ...
            {
                Logger("Falha na conexão : " + ToJson(response));
                Logout();
            }
        });
    }
    ...
    {
        GameObject.Find("ButtonRegister").GetComponent<Button>().onClick.AddListener(async () =>
        {
            GameObject RegisterModal = Resources.Load<GameObject>("Res_HUDLogin/RegisterModal") as GameObject;
            RegisterModal = Instantiate(RegisterModal, new Vector3(Screen.width / 2, Screen.height / 2, 0), Quaternion.identity,GameObject.Find("HomeScreen").transform);
            RegisterModal.name = NoCloneName(RegisterModal.name);
        });
    }
